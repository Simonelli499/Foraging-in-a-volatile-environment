...
# Preparing the data
data <- read_excel("/Users/valeriasimonelli/Desktop/PostDoc/Research/Foraging_Volatilitity/Adults/PulizieDiPrimavera/df_viol_same.xlsx")
...
g <- ggplot() +
  ...
  labs(
    x = "block",
    y = "good boxes rule violation same trial"
  ) +
  theme_classic()
...
# here we try GAUSSIAN
set.seed(123)
m000 = glmer(totalboxes ~ block*AQmediansplit + (1|Subject),
             data = data,family = gaussian(link = identity), control=optimizer) 
# Selected model:
sel_mod = m000
summary(sel_mod)
...
# save the plot
ggsave(
  filename = "/Users/valeriasimonelli/Desktop/PostDoc/Research/Foraging_Volatilitity/Adults/Analisi_R/RuleViolation/boxestotal_ruleviolationsame_AQ.png.png",
  plot = g,
  width = 8,
  height = 6,
  dpi = 300
