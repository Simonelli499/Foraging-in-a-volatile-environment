...
# ------------------------ GOOD BOXES (looking at DATA from all participants and without + icar) -------------------------------
...
data$AQ_group <- factor(
  as.character(data$AQmediansplit),
...
# ------------------------ GOOD BOXES (looking at DATA from all participants that did the icar WITH + icar) -------------------------------
...
# Recode AQ grouping variable for clearer output labels
data_ICARsubjects$AQmediansplit <- factor(
...
slopes_trial <- emtrends(
  ...
  ~ AQmediansplit | block,   # slopes estimated per group within each block
  var = "trial",             # variable for which the slope is computed
  at = list(block = unique(data_ICARsubjects$block))  # evaluate slopes at each observed block
)
...
data_ICARsubjects$AQ_group <- factor(
  as.character(data_ICARsubjects$AQmediansplit),
  levels = c("low AQ", "high AQ")
)
